...
- Ops efímeras (`cursor.move`, `ping`, `camera.sync`, `presence.rename`) nunca persisten ni entran al undo.
- Capas fijas: fondo → grilla → dibujos → AoE → tokens.
- Uploads: 10 MB máx, recompresión a WebP (mapas y fills ≤4096 px, tokens ≤512 px), servidos inmutables. `kind=fill` = rellenos del balde: no se listan como mapas.
